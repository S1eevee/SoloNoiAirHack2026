"""
Departures gate bell curve feature engineering.

Passengers reach the boarding gate ~45 min before departure.
The cluster is tighter than check-in (σ = 15 min vs 30 min for check-in)
because boarding announcements drive a more synchronized rush.
"""
import numpy as np
import pandas as pd
import yaml
...
GATE_LEAD_MIN   = 45.0   # minutes before STD passengers arrive at gate
GATE_SIGMA_MIN  = 15.0   # tighter than check-in's 30 min
GATE_TAIL_SIGMA = 2.0
...
def _spread_gate_departures(df: pd.DataFrame, freq: str = "30min") -> pd.DataFrame:
    dep = df[~df["flight_id"].str.endswith("_ARR", na=False)].copy()
    if dep.empty:
        return dep

    sigma_s  = GATE_SIGMA_MIN * 60.0
    half_win = pd.Timedelta(freq) / 2
    tail     = pd.Timedelta(seconds=sigma_s * GATE_TAIL_SIGMA)

    rows = []
    for _, flight in dep.iterrows():
        # center is BEFORE departure (passengers rush to gate 45 min out)
        mean_t    = flight["scheduled_time"] - pd.Timedelta(minutes=GATE_LEAD_MIN)
...
def build_departures_gate_feature_matrix(df: pd.DataFrame, freq: str = "30min") -> pd.DataFrame:
    peak_hours = _load_peak_hours()

    df = _spread_gate_departures(df, freq=freq)
    if df.empty:
        return pd.DataFrame(columns=DEPARTURES_GATE_FEATURE_COLS + ["window_start", TARGET_COL])
